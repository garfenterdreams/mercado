...
if [ "$TABLE_COUNT" = "0" ] || [ -z "$TABLE_COUNT" ]; then
    ...
    mysql -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PASS" -e "CREATE DATABASE IF NOT EXISTS ${DB_NAME};" 2>/dev/null || true

    # Import the SQL schema
    if [ -f /spurtcommerce-api/init.sql ]; then
        echo "Importing spurtcommerce schema (this may take a few minutes)..."
        mysql -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" < /spurtcommerce-api/init.sql
